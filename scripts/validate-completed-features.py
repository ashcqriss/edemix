...
def main() -> int:
    require("shared/includes/usr/libexec/edemint-inspector", (
        "subprocess.run(", "timeout=timeout_seconds()", "SANDBOX",
        "Open Disposable Sandboxed Viewer", "mime_mismatch",
    ))
    require("shared/includes/usr/libexec/edemint-inspector-probe", (
        "MAX_PREVIEW_BYTES = 65536", "os.O_NOFOLLOW", "stat.S_ISREG",
        "file", "pass_fds=(descriptor,)", "mime_mismatch",
    ))
    ...
    require("shared/includes/usr/libexec/edemint-activity-monitor-app", (
        ...
        "signal.SIGTERM", "os.setpriority", "gpu_snapshot",
        "temperature_snapshot", "Exported JSON and CSV history",
    ))
    ...
    require("shared/includes/usr/libexec/edemint-mission-control-app", (
        'hypr_json("workspaces")', 'hypr_json("clients")',
        'dispatch("focuswindow"', 'dispatch("closewindow"',
        'dispatch("movetoworkspacesilent"', "Remove Empty",
    ...
    require("scripts/smoke-hyprland-session.sh", (
        "AQ_DRM_DEVICES", "MESA_LOADER_DRIVER_OVERRIDE=kms_swrast", "seatd -g video",
        "Hyprland --config", "hyprctl -j monitors", "edemint-session-smoke",
    ))
    require("profiles/arm64-pi/build.sh", (
        "debian-archive-keyring.gpg", "EDEMINT_DEBIAN_KEYRING",
    ...
    if "localepurge" in package_names("shared/package-lists/base.list.chroot"):
        raise SystemExit("base image must preserve the complete locale dataset")
    if (ROOT / "shared/includes/etc/locale.nopurge").exists():
        raise SystemExit("obsolete localepurge configuration must not be shipped")
    require("shared/package-lists/desktop.list.chroot", (
